...
Download_Server_Status_client() {
  cd "/tmp" || exit 1
  wget -N --no-check-certificate "http://www.jacobabc123.xyz/serverstatus-toyo/status-client.py"
...
Service_Server_Status_server() {
  if [[ ${release} == "centos" ]]; then
    if ! wget --no-check-certificate "http://www.jacobabc123.xyz/serverstatus-toyo/server_status_server_centos" -O /etc/init.d/status-server; then
    ...
    if ! wget --no-check-certificate "http://www.jacobabc123.xyz/serverstatus-toyo/server_status_server_debian" -O /etc/init.d/status-server; then
      echo -e "${Error} ServerStatus 服务端服务管理脚本下载失败 !" && exit 1
    fi
    chmod +x /etc/init.d/status-server
...
Service_Server_Status_client() {
  if [[ ${release} == "centos" ]]; then
    if ! wget --no-check-certificate "http://www.jacobabc123.xyz/serverstatus-toyo/server_status_client_centos" -O /etc/init.d/status-client; then
    ...
    if ! wget --no-check-certificate "http://www.jacobabc123.xyz/serverstatus-toyo/server_status_client_debian" -O /etc/init.d/status-client; then
      echo -e "${Error} ServerStatus 客户端服务管理脚本下载失败 !" && exit 1
    fi
    chmod +x /etc/init.d/status-client
...
Install_jq() {
  ...
  if [[ ! -e ${region_json} ]]; then
    wget --no-check-certificate "http://www.jacobabc123.xyz/serverstatus-toyo/zh.json" -O ${region_json}
...
Install_caddy() {
  ...
  if [[ "${caddy_yn}" == [Yy] ]]; then
    ...
    if [[ ! -e "/usr/local/caddy/caddy" ]]; then
      wget -N --no-check-certificate http://www.jacobabc123.xyz/serverstatus-toyo/caddy_install.sh
...
Update_Shell() {
  sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "http://www.jacobabc123.xyz/serverstatus-toyo/status.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | head -1)
  ...
  wget -N --no-check-certificate "http://www.jacobabc123.xyz/serverstatus-toyo/status.sh" && chmod +x status.sh
