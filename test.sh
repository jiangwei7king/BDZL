...
echo -e "\033[1;32m 开始安装clash \033[0m"

wget --no-check-certificate -O clash-linux-amd64-v1.6.5.gz https://github.com/Dreamacro/clash/releases/download/v1.6.5/clash-linux-amd64-v1.6.5.gz && gunzip /root/clash-linux-amd64-v1.6.5.gz && rm -rf clash-linux-amd64-v1.6.5.gz*
...
if [ ! -f "/opt/clash/clash" ];then
   rm -rf /root/clash-linux-amd64-v1.6.5*
   else
   mv /root/clash-linux-amd64-v1.6.5 /opt/clash/clash
  fi

cd /opt/clash && chmod +x clash

wget --no-check-certificate -O config.yaml https://www.somersaultcloud.xyz/link/XXIzjKTyNe2ed98B?clash=1&extend=1

wget --no-check-certificate -O Country.mmdb https://raw.githubusercontent.com/alecthw/mmdb_china_ip_list/release/Country.mmdb

ll

touch /etc/systemd/system/clash.service

cd /etc/systemd/system && rm -rf clash.service* && echo > clash.service
...
sed -i 's/.*http_proxy.*//' /etc/profile

echo 'export http_proxy=http://127.0.0.1:7890' | cat - /etc/profile > temp && echo y | mv temp /etc/profile

sed -i 's/.*https_proxy.*//' /etc/profile

echo 'export https_proxy=http://127.0.0.1:7890' | cat - /etc/profile > temp && echo y | mv temp /etc/profile

source /etc/profile

echo > /root/autoupdate.sh && chmod +x autoupdate.sh

#!/bin/bash
cat > /root/autoupdate.sh <<EOF
ps -ef | grep clash.service | grep -v grep | awk '{print $2}' | xargs kill -9

cd /opt/clash && wget --no-check-certificate -O config.yaml https://www.somersaultcloud.xyz/link/XXIzjKTyNe2ed98B?clash=1&extend=1
...
sed -i 's/.*autoupdate.sh.*//' /var/spool/cron/root

echo '0 */6 * * * sh /root/autoupdate.sh' | cat - /var/spool/cron/root > temp && echo y | mv temp /var/spool/cron/root && service crond reload
