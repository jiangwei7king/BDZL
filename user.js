...
var setting = {
        ...
        ,rate: '16' // 视频播放默认倍率，参数范围0∪[0.0625,16]，'0'为秒过，默认'1'倍
        ...
        ,username: '200202040123' // 学号/工号/借书证号(邮箱/手机号/账号)，例如'2018010101'，默认''
        ,password: 'JYD13138012436' // 密码，例如'123456'，默认''
    },
    _self = unsafeWindow,
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if ((obj.code==1)||(obj.code==0)||obj.tm) {
                    ...
                    obj.data = /^http/.test(data) ? '<img src="' + data + '">' : obj.answer;
                    $(
                        '<tr>' +
                ...
                if(obj.msg) obj.msg+='<br/>🐴德，突然硬不起來了';
                else obj.msg = '<br/>章魚哥草泥馬';
...
function submitAnswer($job, data) {
    ...
    data.length && GM_xmlhttpRequest({
        method: 'POST',
        url: 'http://report.902000.xyz:88/report/cx/0/?workRelationId=' + $('#workId').val(),
        headers: {
            'Content-type': 'application/x-www-form-urlencoded'
        },
