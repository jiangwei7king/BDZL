...
// @match        *://*.chaoxing.com/*
// @match        *://*.edu.cn/*
// @connect      onlinecoursekiller.online
// @run-at       document-end
// @grant        unsafeWindow
...
function beforeFind() {
    ...
    setting.div = $(
        '<div style="border: 2px dashed rgb(190 ,231 ,233); width: 330px; position: fixed; top: 0; right: 0; z-index: 99999; background-color: rgba(190, 231, 233 ,0.6); overflow-x: auto;">' +
    ...
    GM_xmlhttpRequest({
        method: 'GET',
        url: 'http://onlinecoursekiller.online/OnlineCourseKiller/getNotice',
...
function findAnswer() {
    var url = "http://onlinecoursekiller.online/OnlineCourseKiller/killer";
    ...
    GM_xmlhttpRequest({
            ...
            if (!setting.loop) {
                ...
                if (true){
                    ...
                    $(
                        ...
                        '</tr>'
                    ).appendTo(setting.div.find('tbody')).css('background-color', fillAnswer($TiMu.find('ul:eq(0)').find('li'), obj, type) ? '' : 'rgba(190 ,231 ,233, 0.6)');
        ...
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '服务器超时...');
        }
    });
}
...
function submitAnswer($job, data) {
    ...
    data.length && GM_xmlhttpRequest({
        method: 'POST',
        url: 'http://onlinecoursekiller.online/OnlineCourseKiller/uploadCourse',
        headers: {
            'Content-type': 'application/x-www-form-urlencoded'
        },
        data: 'course=' + encodeURIComponent(setting.curs) + '&data=' + encodeURIComponent((Ext.encode || JSON.stringify)(data)) + '&id=' + $('#jobid').val().slice(5)+'&courseId='+
$('#courseId').val()    });
