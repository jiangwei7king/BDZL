...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://flashproxy.cn/soga/latest.json" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://flashproxy.cn/soga/core/soga-cracked-linux64.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://flashproxy.cn/soga/core/soga-cracked-linux64.tar.gz"
    ...
    if [[ ! -f /etc/soga/soga.conf ]]; then
        ...
        echo -e ""
        echo -e "RManLuo 大佬破解 FlashProxy搬运修改"
    else
    ...
    curl -o /usr/bin/soga -Ls https://flashproxy.cn/soga/install/soga.sh
