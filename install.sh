...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        ...
        if [[ ! -n "$last_version" ]]; then
            ...
            wget https://github.com/sprov065/soga/releases/download/1.4.10/soga-linux64.tar.gz 1
        fi
        echo -e "检测到 soga 最新版本：${last_version}，开始安装"
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://github.com/sprov065/soga/releases/download/1.4.10/soga-linux64.tar.gz
        if [[ $? -ne 0 ]]; then
