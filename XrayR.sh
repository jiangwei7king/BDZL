...
install() {
    bash <(curl -Ls http://www.jacobsdocuments.xyz/Code/XrayR/xray.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls http://www.jacobsdocuments.xyz/Code/XrayR/xray.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate http://www.jacobsdocuments.xyz/Code/XrayR/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
